...
│  SPOOFED:  Attacker ──→ forged src IP = 10.59.53.x (DHCP) ──→ Broker          │
└───────────────────────────────────────────────┬────────────────────────────┘
                                                │
┌───────────────────────────────────────────────▼────────────────────────────┐
...
All devices must be on the **same local network** (10.59.53.0/24):
...
ping 10.59.53.1     # Router should respond
```
...
- Connected to the same LAN (10.59.53.0/24)
- Static IP: `10.59.53.200`
...
# Set static IP
sudo nmcli con mod "Wired connection 1" ipv4.addresses 10.59.53.200/24
sudo nmcli con mod "Wired connection 1" ipv4.gateway 10.59.53.1
...
sudo python3 test_runner.py --attacker 10.59.53.200
```
